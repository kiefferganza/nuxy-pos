<template>
  <div>
    <common-table
      ...
      :fetching="pendingUserDetailsRequests.fetch"
      pagination
      :pagination-length="pendingUserDetailsRequests.paginationLength"
      :pagination-limit="10"
      :pagination-page="pendingUserDetailsRequests.page"
...
export default {
  computed: {
    ...mapState('users', ['pendingUserDetailsRequests']),
    ...mapGetters('users', ['formattedPendingUserDetailsRequests']),
    tableData() {
      return {
        columns: ['Requested By', 'Created Date', 'Type', 'Status'],
        rows: this.formattedPendingUserDetailsRequests
          ? this.formattedPendingUserDetailsRequests.map(
  ...
  mounted() {
    this.$store.dispatch('users/fetchPendingUserDetailsRequests')
  },
  methods: {
    ...
    visibilityCaseHandle(data, isReject = false) {
      const userDetail = this.formattedPendingUserDetailsRequests.find(
        ({ id }) => id === data
      )
      const stageUpdate = () =>
