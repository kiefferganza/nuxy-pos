<template>
  <div>
    <common-table
      ...
      :fetching="userLockRequest.fetchUserLockRequestState"
      pagination
      :pagination-length="userLockRequest.paginationLength"
      :pagination-limit="10"
      :pagination-page="userLockRequest.page"
...
export default {
  computed: {
    ...
    tableData() {
      return {
        ...
        rows: this.formattedUserLockRequest
          ? this.formattedUserLockRequest.map(
              ({
                requestedBy,
                recordStatus,
                id,
  ...
  methods: {
    ...mapActions('users', ['fetchLockRequests']),
    ...mapMutations('users', ['setUserLockCurrentPage']),
    visibilityCaseHandle(data, isReject = false) {
      const userLockRequest = this.formattedUserLockRequest.find(
        ({ id }) => id === data
      )
      const stageCreate = () => this.$authorize('/users/lock/update')
      return isReject
        ? stageCreate() && userLockRequest.RecordStatus === 0
        : stageCreate()
    },
    changePage(page) {
      this.setUserLockCurrentPage(page)
      this.fetchLockRequests()
    },
  },
}
</script>
